...
```{r get_data}
samples <- paste0(c('AO15','AO16','AO22'),'_HT2_16h_control')
...
write.csv(markers,file.path('..','data','seurat','organoids_HT2_markers.csv'),row.names=FALSE)
...
anchors <- FindTransferAnchors(reference = travaglini, query = sobj, dims = 1:30,
                               features = intersect(row.names(travaglini@assays$RNA@scale.data),
                                                    row.names(sobj@assays$integrated@scale.data)))
pred <- TransferData(anchorset = anchors, refdata = travaglini$free_annotation, dims = 1:30, k.weight=35)
...
```{r save, cache=FALSE}
saveRDS(sobj,file='../data/seurat/organoids_HT2.rds')
```
