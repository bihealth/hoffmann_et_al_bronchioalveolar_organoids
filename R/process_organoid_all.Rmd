...
```{r get_data}
samples <- c(outer(paste(outer(c('AO15','AO16','AO22'),c('HT2','pool'),FUN=paste,sep='_'),
                           '16h',sep='_'),
                   c('control','H3N2'),FUN=paste,sep='_'))
...
sobj[['protocol']] <-  gsub('(AO[0-9]*)-([poolHT280]*)-([516hd]*)-([controlH3N2]*)','\\2',
                         sobj@meta.data$orig.ident)
sobj[['batch']] <-  ifelse(sobj$donor %in% c('AO16','AO22'),
                           'batch4', gsub('16h','batch3',
                                          gsub('AO[0-9]*-[poolHT280]*-([16hbatch12]*)-[controlH3N2]*',
                                               '\\1',
                                               sobj$orig.ident)))
sobj[['infect']] <-  gsub('(AO[0-9]*)-([poolHT280]*)-([516hd]*)-([controlH3N2]*)','\\4',
                         sobj@meta.data$orig.ident)
...
write.csv(markers,file.path('..','data','seurat','organoids_all_markers.csv'),row.names=FALSE)
...
cds <- order_cells(cds, reduction_method = "UMAP",
                   root_cells = Cells(sobj)[sobj$seurat_clusters=='0'])
...
```{r pseudobulk}
expr <- list()
for (cluster in unique(Idents(sobj))) {
  for (sample in unique(sobj@meta.data$orig.ident)) {
    cells <- Cells(sobj)[(sobj@meta.data$orig.ident==sample) & (Idents(sobj)==cluster)]
...
colData <- data.frame(cell.type=factor(gsub('_.*','',names(expr))),
                      ...
                      protocol=factor(gsub('[^_]*_(AO[0-9]*)-([poolHT280]*)-([516hd]*)-([controlH3N2]*)',
                                         '\\2',names(expr)),
                                    levels=c('pool','HT2')),
                      infect=factor(gsub('[^_]*_(AO[0-9]*)-([poolHT280]*)-([516hd]*)-([controlH3N2]*)',
                                         '\\4',names(expr)),
                                    levels=c('control','H3N2')),
                      group=factor(gsub('[^_]*_(AO[0-9]*)-([poolHT280]*)-([516hd]*)-([controlH3N2]*)',
                                         '\\2_\\4',names(expr)),
                                    levels=c('pool_control','HT2_control','pool_H3N2','HT2_H3N2')),
                      group=names(expr),
...
```{r DE}
res <- list()
for (cluster in c("all",as.character(unique(Idents(sobj))))) {
  ...
  dds <- DESeq(dds)
  res[[paste0(cluster,'_pool')]] <- lfcShrink(dds,contrast=c('group','pool_H3N2','pool_control'),
                                              type='normal',
                                              format="DataFrame") %>%
    as.data.frame() %>%
    tibble::rownames_to_column('gene_name') %>%
    dplyr::select(-stat) %>%
    dplyr::mutate(cluster=cluster,contrast='pool')
  res[[paste0(cluster,'_HT2')]] <- lfcShrink(dds,contrast=c('group','HT2_H3N2','HT2_control'),
                                             type='normal',
                                             format="DataFrame") %>%
    as.data.frame() %>%
    tibble::rownames_to_column('gene_name') %>%
    dplyr::select(-stat) %>%
    dplyr::mutate(cluster=cluster,contrast='HT2')
  
  design(dds) <- ~donor+protocol+infect #+protocol:infect
  dds <- DESeq(dds)
  res[[paste0(cluster,'_protocol')]] <- lfcShrink(dds,contrast=c('protocol','HT2','pool'),
                                                  type='normal',
                                                  format="DataFrame") %>%
    as.data.frame() %>%
    tibble::rownames_to_column('gene_name') %>%
    ...
    dplyr::select(-stat)
  res[[paste0(cluster,'_infect')]] <- lfcShrink(dds,contrast=c('infect','H3N2','control'),
                                                type='normal',
                                                format="DataFrame") %>%
    as.data.frame() %>%
    tibble::rownames_to_column('gene_name') %>%
    dplyr::mutate(cluster=cluster,contrast='infect') %>%
    dplyr::select(-stat)
}

write.csv(do.call(rbind,res),file=file.path('..','data','seurat','organoids_all_pseudobulk_DE.csv'))
```

```{r save, cache=FALSE}
saveRDS(sobj,file='../data/seurat/organoids_all.rds')
```
